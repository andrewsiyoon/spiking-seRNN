...
for epoch in range(num_epochs):
    for i, (data, targets) in enumerate(iter(trainloader)):
        ...
        L1_loss = regu_strength * torch.sum(abs_weight_matrix)

        #Calculate loss
        loss_val = loss_fn(spk_outputs, targets) + L1_loss
    ...
    with torch.no_grad():
        ...
        for data, targets in testloader:
            ...
            test_abs_weight_matrix = torch.abs(net.lif1.recurrent.weight)
            test_L1_loss = regu_strength * torch.sum(test_abs_weight_matrix)

            _, predicted = test_spk.sum(dim=0).max(1)
            total += targets.size(0)
            correct += (predicted == targets).sum().item()
            test_loss = loss_fn(test_spk, targets) + test_L1_loss
...
#Set up directories -----
run_id = datetime.now()
run_path = "/imaging/shared/users/ja02/CBUActors/MPhil/Andrew/wrapper_seRSNN/L1_run_" + run_id.strftime("%Y_%m_%d")
